...
#ifdef LIN_FCUART_8_ISR_USED
extern void Lin_IsrTxRx_FCUART_8(void);
void FCUART8_RxTx_IRQHandler(void)
{
    Lin_IsrTxRx_FCUART_8();
}
#endif
...
void Bsp_Lin_Init(void)
{
    ...
    {
        IntMgr_SetPriority(FCUART8_IRQn, 1);
        IntMgr_EnableInterrupt(FCUART8_IRQn);
    }
}
#else
void Bsp_Lin_Init(void)
{
    ...
    IntMgr_EnableInterrupt(FCUART7_IRQn);
    IntMgr_SetPriority(FCUART8_IRQn, 4);
    IntMgr_EnableInterrupt(FCUART8_IRQn);
}
#endif
...
)
{
   if(PduPtr->Pid == 0x85)
    {
        static uint8 TxBuffer[8] = {0x67, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08};
