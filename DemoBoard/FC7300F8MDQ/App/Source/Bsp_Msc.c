...
static uint16 s_u16Rgb1Step       = PWM_MAX_DUTY / 50;
...
static void Bsp_Msc_SendStart(void)
{
    uint32   u32Temp = 0;
...
static void Bsp_Msc_ReadConfigOutputEven(void)
{
    uint32   u32Temp = 0;
...
static void Bsp_Msc_ReadConfigOutputOdd(void)
{
    uint32   u32Temp = 0;
...
static void Bsp_Msc_ReadData6(void)
{
    uint32   u32Temp = 0;
...
void Bsp_Msc_Init(void)
{
	if(1U == GET_CPU_ID())
	{
		...
		if(CoreId==0U)
		{
            IRQRouter_SetNVIC(IRQROUTER_CPU0_CPU1_CPU2, MSC1_IRQn, FALSE);
            IRQRouter_SetNVIC(IRQROUTER_CPU1, MSC1_IRQn, TRUE);
            SCM->MSC1_ROUTING = 0x22220000U;
        }
		else if (CoreId==1U)
		{
			IntMgr_SetPriority(MSC1_IRQn, 4);
			IntMgr_EnableInterrupt(MSC1_IRQn);
		}
	#else
		IntMgr_SetPriority(MSC1_IRQn, 4);
		IntMgr_EnableInterrupt(MSC1_IRQn);
...
static void Bsp_Msc_Set_Pwm_20ms_Task_Event(void)
{
	if(1U == GET_CPU_ID())
	{
