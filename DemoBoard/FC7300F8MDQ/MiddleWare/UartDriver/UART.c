...
static uint8 FCUART_Int64ToChar(uint64            u64Value,
                                char             *pOutStr,
                                UART_PrintIntType eRadix,
                                boolean           bHexUpper,
                                boolean           bSigned);
...
/**
 * @brief  Convert 64-bit integer to char
 *
 * @param  u64Value: 64-bit integer value
 * @param  pOutStr: Output string buffer
 * @param  eRadix: Number base (10/16)
 * @param  bHexUpper: Hexadecimal uppercase flag
 * @param  bSigned: Signed number flag
 * @return 0 is ok
 */
static uint8 FCUART_Int64ToChar(uint64            u64Value,
                                char             *pOutStr,
                                ...
                                boolean           bSigned)
{
    ...
    else
    {
        ...
        if ((UART_PRINT_RADIX_DEC == eRadix) && bSigned)
        {
            /* Signed decimal number, check if negative */
            if ((sint64)u64Value < 0)
            {
                u64Temp            = (uint64)(0ULL - (sint64)u64Value);
                pOutStr[u8Index++] = '-';
                u8ValueStart       = 1U;
                pCharList          = aNumList;
            }
        ...
        {
            /* Unsigned number or hexadecimal */
            if (TRUE == bHexUpper)
            {
                pCharList = aCharListUpper;
            }
        ...
        /* Convert data to string and store in reverse order */
        do
        {
            pOutStr[u8Index++] = pCharList[u64Temp % (uint8)eRadix];
            u64Temp /= (uint8)eRadix;
        }
        while (u64Temp);
