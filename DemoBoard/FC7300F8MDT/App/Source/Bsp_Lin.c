...
void Bsp_Lin_Init(void)
{
    ...
    #endif
    #if (LIN_PRECOMPILE_SUPPORT == STD_ON)
    (void)Lin_WakeupInternal(Lin_PBCfgVarPredef[CoreId]->pLin_CoreConfig[0]->pLinChannel->u8LinChannelID);
    (void)Lin_WakeupInternal(Lin_PBCfgVarPredef[CoreId]->pLin_CoreConfig[2]->pLinChannel->u8LinChannelID);
    (void)Lin_WakeupInternal(Lin_PBCfgVarPredef[CoreId]->pLin_CoreConfig[3]->pLinChannel->u8LinChannelID);
    #else
    ...
    (void)Lin_WakeupInternal(Lin_Config[CoreId]->pLin_CoreConfig[2]->pLinChannel->u8LinChannelID);
    (void)Lin_WakeupInternal(Lin_Config[CoreId]->pLin_CoreConfig[3]->pLinChannel->u8LinChannelID);
    #endif
    if (CoreId == 0)
    {
        IntMgr_SetPriority(FCUART6_IRQn, 1);
        IntMgr_EnableInterrupt(FCUART6_IRQn);
        IntMgr_SetPriority(FCUART2_IRQn, 4);
        IntMgr_EnableInterrupt(FCUART2_IRQn);
    }
    else if (CoreId == 1)
    {
        IntMgr_SetPriority(FCUART7_IRQn, 1);
        IntMgr_EnableInterrupt(FCUART7_IRQn);
    }
    else if (CoreId == 2)
    {
        IntMgr_SetPriority(FCUART17_IRQn, 1);
        IntMgr_EnableInterrupt(FCUART17_IRQn);
    }
}
#else
void Bsp_Lin_Init(void)
{
    ...
    Lin_Init(NULL_PTR);
    #else
    Lin_Init(Lin_Config[0]);
    #endif
    (void)Lin_WakeupInternal(Lin_Config[0]->pLin_CoreConfig[0]->pLinChannel->u8LinChannelID);
    (void)Lin_WakeupInternal(Lin_Config[0]->pLin_CoreConfig[2]->pLinChannel->u8LinChannelID);
    (void)Lin_WakeupInternal(Lin_Config[0]->pLin_CoreConfig[3]->pLinChannel->u8LinChannelID);
    (void)Lin_WakeupInternal(Lin_Config[0]->pLin_CoreConfig[1]->pLinChannel->u8LinChannelID);
    IntMgr_SetPriority(FCUART6_IRQn, 4);
    IntMgr_SetPriority(FCUART2_IRQn, 4);
    IntMgr_EnableInterrupt(FCUART6_IRQn);
    IntMgr_EnableInterrupt(FCUART2_IRQn);
    IntMgr_SetPriority(FCUART7_IRQn, 4);
    IntMgr_EnableInterrupt(FCUART7_IRQn);
    IntMgr_SetPriority(FCUART17_IRQn, 4);
    IntMgr_EnableInterrupt(FCUART17_IRQn);
}
#endif

#if LIN_MULTICORE_SUPPORT == STD_ON
void Bsp_Lin_Core0_1s_Task_Event(void)
{
...
void Bsp_Lin_Core1_1s_Task_Event(void)
{
    uint32 CoreId = GET_CPU_ID();
    Lin_PduType tPduData;
    static uint8 aBuffer[8] = {0x1, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77};
    static uint8 u8Count = 1;
    uint8 u8ChannelId = 0;
    if (CoreId == 1)
    {
        #if (LIN_PRECOMPILE_SUPPORT == STD_ON)
        u8ChannelId = Lin_PBCfgVarPredef[CoreId]->pLin_CoreConfig[3]->pLinChannel->u8LinChannelID;
        #else
        u8ChannelId = Lin_Config[CoreId]->pLin_CoreConfig[3]->pLinChannel->u8LinChannelID;
        #endif
        tPduData.Pid = 0xA7U;
        tPduData.Cs = LIN_ENHANCED_CS;
        tPduData.Drc = LIN_FRAMERESPONSE_TX;
...
void Bsp_Lin_Core2_1s_Task_Event(void)
{
    uint32 CoreId = GET_CPU_ID();
    Lin_PduType tPduData;
    static uint8 aBuffer[8] = {0x1, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07};
    static uint8 u8Count = 1;
    uint8 u8ChannelId = 0;
    if (CoreId == 2)
    {
        #if (LIN_PRECOMPILE_SUPPORT == STD_ON)
        u8ChannelId = Lin_PBCfgVarPredef[CoreId]->pLin_CoreConfig[2]->pLinChannel->u8LinChannelID;
        #else
        u8ChannelId = Lin_Config[CoreId]->pLin_CoreConfig[2]->pLinChannel->u8LinChannelID;
        #endif
        tPduData.Pid = 0xA8U;
        tPduData.Cs = LIN_ENHANCED_CS;
        tPduData.Drc = LIN_FRAMERESPONSE_TX;
...
void Bsp_Lin_1s_Task_Event(void)
{
	if(0U == GET_CPU_ID())
	{
        Bsp_Lin_Core0_1s_Task_Event();

	}
	else if(1U == GET_CPU_ID())
	{
        Bsp_Lin_Core1_1s_Task_Event();
	}
	else if(2U == GET_CPU_ID())
	{
		Bsp_Lin_Core2_1s_Task_Event();
	}
}
#else
void Bsp_Lin_1s_Task_Event(void)
