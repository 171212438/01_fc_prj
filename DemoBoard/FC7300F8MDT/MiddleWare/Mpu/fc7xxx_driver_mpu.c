...
{
    MCAL_DATA_MEMORY_BARRIER(); /* make sure outstanding transfers are done */
...
void MPU_Enable(MPU_EnableOptionType eOption)
{
    ...
    MPU_HWA_Set_CR(CORTEX_MPU_CTRL_ENABLE_MASK | (((uint32)u32Option) & MPU_EN_MASK_U32));
...
MPU_StatusType MPU_RegionEnable(MPU_RegionNumberType eRegion, const MPU_RegionConfigurationType *pConfig)
{
    ...
    if ((NULL_PTR == pConfig)              ||
        ((uint32)eRegion > (uint32)MPU_REGION_NUMBER_15) ||
        (0U != (pConfig->u32BaseAddr & MPU_RBAR_VALID_REGION_MASK_U32)) ||
		((uint32)pConfig->eRegionSize < (uint32)MPU_REGION_SIZE_32B) ||
		((uint32)pConfig->eRegionSize > (uint32)MPU_REGION_SIZE_4GB))
    {
    ...
    else
    {
        ...
        u32Srd =    (((((uint32)pConfig->eSubRegionDis_0) << 0) |
                      (((uint32)pConfig->eSubRegionDis_1) << 1) |
                      (((uint32)pConfig->eSubRegionDis_2) << 2) |
                      (((uint32)pConfig->eSubRegionDis_3) << 3) |
                      (((uint32)pConfig->eSubRegionDis_4) << 4) |
                      (((uint32)pConfig->eSubRegionDis_5) << 5) |
                      (((uint32)pConfig->eSubRegionDis_6) << 6) |
                      (((uint32)pConfig->eSubRegionDis_7) << 7)) & 0xFFU);
...
{
    uint32       u32RegVal = MPU_HWA_Get_Type();
    MPU_StatusType eRet      = MPU_STATUS_SUCCESS;
