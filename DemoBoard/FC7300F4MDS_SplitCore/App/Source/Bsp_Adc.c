...
static boolean Adc3_Group5_Ready = FALSE;
...
Adc_ValueGroupType Adc3_Group5_Buffer[ADC_CFGSET_GROUP_5_CHANNELS]  = { 0 };
Adc_ValueGroupType Adc3_Group5_Results[ADC_CFGSET_GROUP_5_CHANNELS] = { 0 };
...
extern ISR(Adc_ISR_EndGroupConvUnit3);
void ADC3_IRQHandler(void)
{
    Adc_ISR_EndGroupConvUnit3();
}
...
void IoHwAb_Adc_Notification_5(void)
{
    Adc3_Group5_Ready = TRUE;
}
...
void Bsp_Adc_20ms_Task_Event(void)
{
    ...
    else if (1 == Cpm_HWA_GetCoreId())
    {
        ...
        if (Adc3_Group5_Ready == TRUE)
        {
            Adc3_Group5_Ready = FALSE;
            Adc_ReadGroup(AdcGroup_5, Adc3_Group5_Results);
            Adc_StartGroupConversion(AdcGroup_5);
        }
    }
}
