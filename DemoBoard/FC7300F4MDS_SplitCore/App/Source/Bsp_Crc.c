...
void Bsp_Crc_Init(void)
{
    if (0 == GET_CPU_ID())
    {
        ...
        uint8 CRC8_SetChannel = (uint8)Crc_CalculateChannel(CrcConf_CrcChannelConfig_CrcChannelConfig_0, &Crc_Config, (const uint8 *)s_aU8InputData, DATASIZE, 0xF3, FALSE);
    ...
    else if (1 == GET_CPU_ID())
    {
        ...
        uint32 CRC32_SetChannel = (uint32)Crc_CalculateChannel(CrcConf_CrcChannelConfig_CrcChannelConfig_1, &Crc_Config, (const uint8 *)s_aU8InputData, DATASIZE, 0xb76d90f4, FALSE);
