...
void Tlib_Dma_Channel0_Init(void)
{
    ...
    /* PRQA S 2982 ++ #Misra-C:2012 Rule-8.4 A compatible declaration shall be visible when an object
     * or function with external linkage is defined. Reason: It is just for testing or demonstration */
    ...
    {
        ret = Dma_ConfigChannel(DMA_INSTANCE_0, 1, &TlibChannelConfig);
    }
    else
    {
...
void Bsp_Dma_Init(void)
{
    ...
    else if (1 == Cpm_HWA_GetCoreId())
    {
        ...
        IntMgr_EnableInterrupt(DMA4_IRQn);
        Tlib_Dma_Channel0_Init();
        Dma_StartChannel(DMA_INSTANCE_0, 1);
    }
}
...
void Bsp_Dma_20ms_Task_Event(void)
{
    ...
    {
        Dma_StartChannel(DMA_INSTANCE_0, 1);
        test_outloopcnt = Dma_GetCurrentOuterLoopCounter(DMA_INSTANCE_0, 3);
    }
    else
    {
        Dma_StartChannel(DMA_INSTANCE_0, DMA_TEST_SW_CHANNEL_ID);
