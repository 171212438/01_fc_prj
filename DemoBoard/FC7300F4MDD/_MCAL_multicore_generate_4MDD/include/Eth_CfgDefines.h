...
*
*/
#define ETH_FATAL_ERROR_CALLBACK_NOTIFICATION       (STD_OFF)

/**
* @brief The Eth transceiver needs to be configured during Eth initialization.
*
*/
#define ETH_TRANSCEIVER_PREINIT                     (STD_ON)
...
*
*/
#define ETH_TIMEOUT_LOOP_COUNT                      (15000U)
...
*
*/
#define ETH_CTRL_ENABLE_OFFLOAD_CHECKSUM            (STD_OFF)
...
/**
 * @brief         Function pointer type for writing MII register.
 * @details       This type defines a function pointer for writing to MII (Media Independent
 * Interface) registers of Ethernet transceivers.
 * @implements    Eth_WriteMiiType_typedef
 */
typedef Std_ReturnType (*Eth_WriteMiiType)(uint8 CtrlIdx, uint8 TrcvIdx, uint8 RegIdx, uint16 RegVal);

/**
 * @brief         Function pointer type for reading MII register.
 * @details       This type defines a function pointer for reading from MII (Media Independent
 * Interface) registers of Ethernet transceivers.
 * @implements    Eth_ReadMiiType_typedef
 */
typedef Std_ReturnType (
    *Eth_ReadMiiType)(uint8 CtrlIdx, uint8 TrcvIdx, uint8 RegIdx, uint16 *RegValPtr);

/**
 * @brief         Function pointer type for writing MMD register.
 * @details       This type defines a function pointer for writing to MMD (Management MDIO)
 * registers of Ethernet transceivers, which are used for extended management functions.
 * @implements    Eth_WriteMmdType_typedef
 */
typedef Std_ReturnType (
    *Eth_WriteMmdType)(uint8 CtrlIdx, uint8 TrcvIdx, uint8 Mmd, uint16 RegIdx, uint16 RegVal);

/**
 * @brief         Function pointer type for reading MMD register.
 * @details       This type defines a function pointer for reading from MMD (Management MDIO)
 * registers of Ethernet transceivers, which are used for extended management functions.
 * @implements    Eth_ReadMmdType_typedef
 */
typedef Std_ReturnType (
    *Eth_ReadMmdType)(uint8 CtrlIdx, uint8 TrcvIdx, uint8 Mmd, uint16 RegIdx, uint16 *RegValPtr);

typedef struct
{
    Eth_ReadMiiType  ReadMii;
    Eth_WriteMiiType WriteMii;
    Eth_ReadMmdType  ReadMmd;
    Eth_WriteMmdType WriteMmd;
} Eth_PreInitMdioFunctionsType;

/**
 * @brief         Function pointer type for transceiver pre-initialization.
 * @details       This type defines a function pointer for transceiver pre-initialization functions
 *                that are called before the main initialization of the Ethernet transceiver.
 * @implements    Eth_TrcvPreInitFunctionTypedef
 */
typedef Std_ReturnType (*Eth_TrcvPreInitFunctionType)(uint8 CtrlIdx, Eth_PreInitMdioFunctionsType *MdioFuncs);
