...
void Bsp_Mcu_ResetReason_Print(void)
{
    ...
    /* Core3 reset reason */
    DEBUG_INFO("Core3 Reset reason value: %x\r\n", *(volatile uint32*)RGM_C3_SRS_ADDR);
}
...
static void Bsp_Mcu_Core0_LowPowerEntry(void)
{
    ...
    do
    {
        ...
        bCore3SleepStatus = (boolean)( ((*(volatile uint32*)SCM_CCM3_STATUS)&SCM_CCM_DEEPSLEEP_MASK)>>SCM_CCM_DEEPSLEEP_SHIFT );
        u32Timeout++;
    } while( ((bCore1SleepStatus == FALSE) || (bCore2SleepStatus == FALSE) || (bCore3SleepStatus == FALSE)) && (u32Timeout < POLLING_TIMEOUT) );
...
static void Bsp_Mcu_Core1_LowPowerEntry(void)
{
    ...
    /* Close all interrupt(including WKU source) */
    for(u8Index=0U; u8Index<(uint8)IRQn_MAX; u8Index++)
    {
        IntMgr_DisableInterrupt((IRQn_Type)u8Index);
    }
...
static void Bsp_Mcu_Core3_LowPowerEntry(void)
{
    uint8 u8Index;
...
void Bsp_Mcu_LowPowerEntry_Task(Bsp_EntryLowPowerCmdType* pCmd)
{
    ...
    {
        *pCmd = BSP_CORE3_ENTRY;
        Bsp_Mcu_Core2_LowPowerEntry();
    }
    else if( (3U == u8CoreId) && (*pCmd == BSP_CORE3_ENTRY) )
    {
        *pCmd = BSP_CORE0_ENTRY;
        Bsp_Mcu_Core3_LowPowerEntry();
    }
