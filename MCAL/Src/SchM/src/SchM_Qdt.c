...
SCHM_DATA_SECTION static uint32 msr_QDT_EXCLUSIVE_AREA[MAX_CORE_NUM];    /**< @brief exclusive area MSR state upon entering */
...
SCHM_DATA_SECTION static volatile uint32 reentry_guard_QDT_EXCLUSIVE_AREA[MAX_CORE_NUM] = { 0U }; /**< @brief guard for the msr variable against overwrite */
...
*
*/
LOCAL_INLINE uint32 Qdt_schm_read_msr(void);
...
==================================================================================================*/
/**
* @brief   This function is called to allow QDT to enter the specific critical region.
...
SCHM_TEXT_SECTION void SchM_Enter_Qdt_QDT_EXCLUSIVE_AREA(void)
{
    uint32 CoreId;
#ifdef MCAL_ENABLE_USER_MODE_SUPPORT
    uint32 msr = Mcal_Trusted_Call_Return(Qdt_schm_read_msr);
#else
    uint32 msr = Qdt_schm_read_msr();  /*read MSR (to store interrupts state) */
    ...
    if (0UL == reentry_guard_QDT_EXCLUSIVE_AREA[CoreId])
    {
        msr_QDT_EXCLUSIVE_AREA[CoreId] = msr;
    }
     ...
     */
    reentry_guard_QDT_EXCLUSIVE_AREA[CoreId]++;
    /* PRQA S 3387 -- */
}
/**
* @brief   This function is called to allow QDT to exit the specific critical region.
...
SCHM_TEXT_SECTION void SchM_Exit_Qdt_QDT_EXCLUSIVE_AREA(void)
{
    ...
    reentry_guard_QDT_EXCLUSIVE_AREA[CoreId]--;
    /* PRQA S 3387 -- */
    if ((ISR_ON(msr_QDT_EXCLUSIVE_AREA[CoreId])) && (0UL == reentry_guard_QDT_EXCLUSIVE_AREA[CoreId]))       /* If interrupts were enabled */
    {
        ResumeAllInterrupts();
#ifdef __CC_ARM
        /* Compiler fix - forces the CSPID instruction to be generated with -02, -Ospace are selected */
